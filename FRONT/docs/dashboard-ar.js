...
function renderFinancialOverviewStats() {
  ...
  /* إجمالي الأسهم */
  const sharesEl = document.getElementById('stat-shares-value');
  if (sharesEl) {
    const totalShares = buildings.reduce((s, b) => s + (typeof b.totalOpShares === 'number' ? b.totalOpShares : 0), 0);
    sharesEl.innerHTML = totalShares > 0 ? totalShares.toLocaleString('ar-SA') + '<small>سهم</small>' : '—<small>سهم</small>';
  }
...
const DASHBOARD_FILTER_HERO = {
  all: { src: 'assets/output-onlinegiftools.gif', alt: 'صورة الإحصاءات العامة' },
  financial: { src: 'assets/money-reports.gif', alt: 'صورة الإحصاءات المالية' },
  administrative: { src: 'assets/law-report.gif', alt: 'صورة الإحصاءات الإدارية' },
  general: { src: 'assets/general-reports.gif', alt: 'صورة الإحصاءات العامة' }
};
...
function handleLogout() {
  if (confirm('هل تريد تأكيد تسجيل الخروج من النظام؟')) {
    window.location.href = window.__ROUTES__.login;
  }
}
...
function setLang(l) {
  ...
  p.lang = l;
  savePrefs(p);
  if (l === 'en') { window.location.href = window.__ROUTES__.homeOther; return; }
...
(function () {
  ...
  function drawBar(data, opts) {
    ...
    data.forEach(function (d, i) {
      ...
      var lb = d.l || '';
      if (lb.length > 4) anyLongLabel = true;
      var lbase = bottomY + (cur ? 22 : 20);
      var lfs = cur ? (lb.length > 6 ? '8.5' : '9.5') : (lb.length > 6 ? '8' : '9');
      var lwords = lb.split(/\s+/);
      if (lwords.length >= 2 && lb.length > 4) {
        var lhalf = Math.ceil(lwords.length / 2);
        var l1 = lwords.slice(0, lhalf).join(' ');
        var l2 = lwords.slice(lhalf).join(' ');
        s += '<text x="' + cx.toFixed(1) + '" y="' + lbase + '" text-anchor="middle" font-size="' + lfs + '" font-weight="500" fill="var(--text-secondary)" font-family="Tajawal">' + escapeCellHtml(l1) + '</text>';
        if (l2) s += '<text x="' + cx.toFixed(1) + '" y="' + (lbase + 13) + '" text-anchor="middle" font-size="' + lfs + '" font-weight="500" fill="var(--text-secondary)" font-family="Tajawal">' + escapeCellHtml(l2) + '</text>';
      } else {
        s += '<text x="' + cx.toFixed(1) + '" y="' + lbase + '" text-anchor="middle" font-size="' + lfs + '" font-weight="500" fill="var(--text-secondary)" font-family="Tajawal">' + escapeCellHtml(lb.slice(0,10)) + '</text>';
      }
    });
    if (cur) {
  ...
  function initAdmCharts() {
    ...
    reg('adm8', function(type) {
      ...
      var data=Object.entries(byP).sort(function(a,b){return b[1]-a[1];}).slice(0,7).map(function(e,i){return{l:e[0],v:e[1],c:GC[i%GC.length]};});
  ...
  function updateGeneral() {
    ...
    /* عدد المدن */
    var allCities=[...new Set(buildings.map(function(b){return (b.city||'').trim();}).filter(Boolean))];
    var citiesEl=document.getElementById('stat-cities-count');
    if(citiesEl) citiesEl.textContent=allCities.length||'—';
  }
