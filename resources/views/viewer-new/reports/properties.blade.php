...
@section('content')
    ...
    <section class="vn-table-card vn-report-detail">
        ...
        @if (($properties ?? collect())->count() > 0)
            <div class="vn-table-responsive">
                <table>
                    <thead><tr><th>#</th><th>المحضر</th><th>المحافظة / المنطقة</th><th>المساحة</th><th>القيمة</th><th>الحالة</th><th>المُلّاك</th><th>العمليات</th><th>ملاحظات</th></tr></thead>
                    <tbody>
                    @foreach ($properties as $property)
                        <tr>
                            <td>{{ $property->id ?? '—' }}</td>
                            <td>{{ ($columns['card_record_number'] ?? false) ? ($property->card_record_number ?: '—') : '—' }}</td>
                            <td>{{ (($columns['card_governorate'] ?? false) ? ($property->card_governorate ?: '—') : '—') }} / {{ (($columns['card_region_name'] ?? false) ? ($property->card_region_name ?: '—') : '—') }}</td>
                            <td>{{ ($columns['card_total_area'] ?? false) ? (filled($property->card_total_area) ? number_format((float) $property->card_total_area, 2) . ' م²' : '—') : '—' }}</td>
                            <td>@php $value = ($columns['total_property_value_usd'] ?? false) ? $property->total_property_value_usd : (($columns['owned_property_value_usd'] ?? false) ? $property->owned_property_value_usd : null); @endphp{{ filled($value) ? number_format((float) $value, 2) . ' $' : '—' }}</td>
                            <td>{{ ($columns['card_status'] ?? false) ? ($property->card_status ?: '—') : '—' }}</td>
                            <td>{{ $property->owners_count ?? '—' }}</td><td>{{ $property->operations_count ?? '—' }}</td>
                            <td class="vn-muted-value">{{ ($columns['card_property_details'] ?? false) ? ($property->card_property_details ?: '—') : '—' }}</td>
                        </tr>
                    @endforeach
                    </tbody>
                </table>
